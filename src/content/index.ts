...
const ikeaProductPageListener = (
  message: IkeaProductPageMessage,
  sender: chrome.runtime.MessageSender,
  sendResponse: (response: IkeaProductPageResponse) => void
) => {
  const productWrapperSelector = document.querySelector(
    "div.pip-page-container__main > div[data-product-id]"
  )!;
  const productId = productWrapperSelector
    .getAttribute("data-product-id")!
    .toLowerCase();
  const productNo = productWrapperSelector.getAttribute("data-product-no")!;
  const productPrice = Number(
    productWrapperSelector.getAttribute("data-product-price")!
  );
