...
function Prices() {
    ...
    const [buttonClassName, setButtonClassName] = React.useState("button is-link");
    ...
    const loadResults = async () => {
        ...
        try {
            ...
            let response: IkeaProductPageResponse = await chrome.tabs.sendMessage(tab.id || 0, {
            ...
            // fetch product data
            const res = await fetch(`https://stores-api-polyu.ondigitalocean.app/api/ikea/products/${response.productId}.json`);
            ...
            for (const filteredPrice of filteredPrices) {
                ...
                if (filteredPrice.currencyCode !== selectedCurrency) {
                    const exchangeRateResponse = await fetch(`https://api.nbp.pl/api/exchangerates/rates/A/${filteredPrice.currencyCode}?format=json`);
                    const exchangeRateResJson = await exchangeRateResponse.json();
                    const exchangeRate = exchangeRateResJson.rates[0].mid;
                    const calculatedPrice = (filteredPrice.price * exchangeRate).toFixed(2);
                ...
                if (response.countryCode !== "pl") {
                    const exchangeRateResponse = await fetch(`https://api.nbp.pl/api/exchangerates/rates/A/${currency(response.countryCode)}?format=json`);
                    const exchangeRateResJson = await exchangeRateResponse.json();
                    const exchangeRate = exchangeRateResJson.rates[0].mid;
                    productPriceToCompare = Number((response.productPrice * exchangeRate).toFixed(2));
                }
                ...
                for (const availability of availabilityResJson.data) {
                    if (availability.classUnitKey.classUnitCode === price.countryCode.toUpperCase()) {
                        continue;
                    }

                    let index = -1;
                    if (availability.isInCashAndCarryRange) {
                        index = availability.availableStocks.findIndex((availableStock: any) => {
                            return availableStock.type === "CASHCARRY";
                        });
                    }
                    const quantity = index >= 0 ? availability.availableStocks[index].quantity : 0;

                    // messageType: "HIGH_IN_STOCK" | "MEDIUM_IN_STOCK" | "LOW_IN_STOCK" | "OUT_OF_STOCK"
                    let messageType = "OUT_OF_STOCK";
                    if (quantity !== 0) {
                        messageType = availability.availableStocks[index].probabilities[0].communication.messageType;
                    }
                    ...
                    if (index >= 0 && quantity === 0) {
                        if (availability.availableStocks[index].restocks) {
                            const restockFromResponse = availability.availableStocks[index].restocks[0];
                            restock = {
                    ...
                    price.availabilities.push({
                        ...
                        storeName: storeNameForStoreId(availability.classUnitKey.classUnitCode),
                        quantity: quantity,
            ...
            if (
                ...
                setSummaryTextContent(`${productName} is cheaper in another country!`);
                setSummaryClassName("title is-5 has-text-centered has-text-danger-dark");
            } else {
                setSummaryTextContent(`${productName} is the cheapest in ${country(response.countryCode)}!`);
                setSummaryClassName("title is-5 has-text-centered has-text-success-dark");
            }
    ...
    return (
        <div id="prices">
            ...
            {showProgressBar && <progress className="progress is-small is-link" max="100"></progress>}
            {showErrorComponent && <ErrorComponent />}
            {showPriceComponent && <PricesSummary summaryTextContent={summaryTextContent} summaryClassName={summaryClassName} />}
            {showPriceComponent && pricesState.map((price, i) => <Price price={price} productName={productNameState} />)}
        </div>
    );
}
