...
const Availability = (props: any) => {
    const availabilityClass = () => {
        ...
        } else if (["MEDIUM_IN_STOCK", "LOW_IN_STOCK"].includes(props.availability.messageType)) {
    ...
    const restockComponent = () => {
        if (props.availability.messageType === "OUT_OF_STOCK") {
            if (props.availability.restock) {
                ...
                return (
                    <span data-tooltip-id={props.availability.storeId + "-restock-tooltip"} data-tooltip-content={restockTooltipContent} className="icon-text has-text-info">
    ...
    return (
        <div id={props.availability.storeId + "-availability"} style={{ borderTop: "0.1rem solid rgb(219, 219, 219" }}>
